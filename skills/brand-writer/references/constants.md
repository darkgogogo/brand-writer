...
**示例**：
- 标题「番茄钟为什么总坚持不到第三个？」→ 目录名 `番茄钟为什么总坚持不到第三个`
- 标题「我花了 30 天测了 20 个生产力工具」→ 目录名 `我花了-30-天测了-20-个生产力工具`
- 标题「FocusFlow 的深度专注模式到底是什么」→ 目录名 `FocusFlow-的深度专注模式到底是什么`

### 其他
- Profile：`<产品英文短名>.md`，小写、连字符或纯小写（例：`focusflow.md`、`my-product.md`）
